...
export default function App() {
  ...
  // Fetch properties from Express backend if available
  useEffect(() => {
    fetch("/api/properties")
      ...
      .catch(() => {
        // Fallback to embedded PROPERTIES_DATA
      });
  }, []);
  ...
  const handleAddNewWoodProperty = async (newProp: PropertyItem) => {
    try {
      ...
      console.warn("Failed to POST to backend, saving in local state", e);
    }
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500">
                  React + Vite + Express + PostgreSQL (Docker Ready)
                </p>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            ...
            <p className="text-slate-300 text-xs max-w-2xl leading-relaxed">
              สถาปัตยกรรม React 19 + Vite + Express + PostgreSQL (Prisma ORM) พร้อม Docker Containerization
            </p>
          </div>
                      ...
                      <div className="pt-3 border-t border-slate-100 flex gap-2">
                        ...
                        {item.dataSourceUrl && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="p-2 bg-amber-50 hover:bg-amber-100 text-amber-700 rounded-xl border border-amber-200 transition flex items-center justify-center"
                            title="เปิดดูบน FastLEDChecker.com"
                          >
